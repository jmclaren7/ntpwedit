...
int open_hives(char *fname[H_COUNT])
{
    ...
    for (i = H_SAM; i < H_COUNT; ++i)
    {
        ...
        else
        {
            ...
            logprintf("open_hives: Hive[%d] '%s' opened\n", i, fname[i]);
        }
    }
...
struct user_info *next_user(struct search_user *su)
{
    ...
    if (ui->rid >= 0)
    {
        char uname_ascii[256];
        /* Convert (best effort) for logging */
        cheap_uni2ascii(ui->unicode_name, uname_ascii, sizeof(uname_ascii) - 1);
        uname_ascii[sizeof(uname_ascii) - 1] = '\0';
        logprintf("list_user: rid=%d name='%s'\n", ui->rid, uname_ascii);
    }
...
/* Detection of Microsoft-connected account.

 */
int is_ms_account(int rid)
{
    /* look in Default hive (we use H_SOFT handle for it)
       at \Software\Microsoft\IdentityCRL\StoredIdentities\
       For each email key, if any subkey name ends with the decimal RID, return 1. */
    if (!hive[H_SOFT])
        return 0;

    int root = trav_path(hive[H_SOFT], 0, "\\Software\\Microsoft\\IdentityCRL\\StoredIdentities\\", 0);
    if (!root)
        return 0;

    char ridstr[16];
    snprintf(ridstr, sizeof(ridstr), "%d", rid);

    struct ex_data ex_email = {0};
    int count = 0, countri = 0;
    while (ex_next_n(hive[H_SOFT], root + 4, &count, &countri, &ex_email) > 0)
    {
        if (ex_email.nk && ex_email.nk->type == 0 && ex_email.nk->len_name > 0)
        {
            /* enumerate subkeys under this email key */
            struct ex_data ex_sub = {0};
            int scount = 0, scountri = 0;
            while (ex_next_n(hive[H_SOFT], ex_email.nkoffs + 4, &scount, &scountri, &ex_sub) > 0)
            {
                if (ex_sub.nk && ex_sub.nk->type == 0 && ex_sub.nk->len_name > 0)
                {
                    char subname[256];
                    int copy = ex_sub.nk->len_name < (int)sizeof(subname) - 1 ? ex_sub.nk->len_name : (int)sizeof(subname) - 1;
                    memcpy(subname, ex_sub.nk->keyname, copy);
                    subname[copy] = 0;
                    size_t sl = strlen(subname), rl = strlen(ridstr);
                    if (rl <= sl && 0 == memcmp(subname + (sl - rl), ridstr, rl))
                    {
                        if (ex_sub.name)
                        {
                            free(ex_sub.name);
...
/* Unlink a Microsoft account by writing a new password hash using change_password.
 * This does not fully sever cloud linkage (needs SECURITY hive edits), but enables offline login. */
int unlink_ms_account(int rid, char *password)
{
    int ms = is_ms_account(rid);
    if (ms <= 0)
        return 0; /* not MS or error */
    if (!change_password(rid, password))
        return 0;
    /* Attempt to purge SECURITY hive cached credentials heuristically. */
    if (hive[H_SEC])
    {
        int nkofs = trav_path(hive[H_SEC], 0, "\\Cache\\", 0);
        if (nkofs)
        {
            struct ex_data ex = {0};
            int count = 0, countri = 0;
            while (ex_next_n(hive[H_SEC], nkofs + 4, &count, &countri, &ex) > 0)
            {
                if (ex.name)
                {
                    free(ex.name);
                ...
                /* We cannot easily map value to specific RID without deep parse;
                   for safety do not delete all. Future: implement proper NL$ parsing. */
            }
        }
    }
    return 1;
